...
export default function VenueDetail({ params }: VenueDetailProps) {
  ...
  const [activeLead, setActiveLead] = useState<{ id: string } | null>(null);
  ...
  const checkForActiveLead = async () => {
    try {
      ...
      // Find the lead for this venue
      const venueLead = leads?.find((lead: any) => lead.venue_id === params.id);
      setActiveLead(venueLead || null);
    } catch (error) {
      console.error('Error checking for active lead:', error);
    }
  };
